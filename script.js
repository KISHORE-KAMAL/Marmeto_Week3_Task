document.addEventListener('DOMContentLoaded', () => {
    ...
    const similarResultsContainer = document.getElementById('recommended');
    const wishlistItemsContainer = document.getElementById('wishlist');
  ...
  const updateLocalStorage = () => {
    const wishlistItems = Array.from(wishlistItemsContainer.children).map((card) => {
      return {
  ...
  loadWishlistFromStorage();
    ...
    const displaySearchResults = (results, isWishlistItem) => {
      const imageContainer = isWishlistItem ? wishlistItemsContainer : similarResultsContainer;
